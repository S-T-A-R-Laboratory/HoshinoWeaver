...
3. **[天地分离](#天地分离)**：带星点对齐的叠加，可分别处理天空和地面区域，产出高信噪比的星空图像。
...
创建星轨双端渐隐效果（类似"半岛雪人"插件），使星轨首尾自然淡入淡出。通过双端滑条控制，左端设置渐入比例，右端设置渐出比例。
...
适合使用大光圈、高感光度拍摄的序列，或星点过于密集时，可通过缩星使输出的星轨线条更纤细、不过于拥堵。
...
提供仅标注天空区域的遮罩图像。如果地景中有明显亮光源（如城市灯光），提供遮罩可提升去卫星线的执行效率和效果，避免地面干扰星点检测。
...
带星点对齐的叠加工作流。算法将所有图像对齐到同一参考帧的星点位置，再分别对天空和地面区域进行叠加，输出高信噪比的星空图像，同时避免地景因旋转产生拖影。

<!-- 建议在此处插入：天地分离工作流示意图（天空对齐→分别叠加→合并） -->

### 准备遮罩文件

天地分离**必须**提供遮罩文件，用于告知软件哪些区域是天空、哪些区域是地面。
...
遮罩文件用于告知软件图像中天空和地面的分界，在去卫星线、噪声均匀化地面融合、以及天地分离工作流中使用。

<!-- 建议在此处插入：遮罩示例图（左：原图，右：对应遮罩） -->
...
- 遮罩尺寸必须与原始图像**完全一致**（像素宽高相同）
- 只需制作一张遮罩，软件会自动应用到整个序列
- 使用 RAW 图像时，遮罩应基于软件实际读取尺寸（非相机显示的缩略图尺寸）制作
