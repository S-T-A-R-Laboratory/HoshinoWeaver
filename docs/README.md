...
- **ShmTransportable 协议**：轻量包装类（如 `FloatImage`）通过 `__shm_pack__` / `__shm_unpack__` 实现 array 走 shm、元数据走 pickle 的混合传输
...
│   │                          #   MaxNoiseEqualizationOp
│   ├── sigma_clip_ops.py      #   DiskBufferWriterOp / SigmaClipIteratorOp
...
│   ├── tagged_image.py        #   FloatImage（含 ShmTransportable）/ dtype 管理 / 缩放工具
