...
## v1.1.0-beta.2 (Sep 16th, 2026)

### ✅ New Features

- **32 位 TIFF 预览**： `uint32` 和 `float32` 的 TIFF 图像现可在 GUI 中预览。
- **去卫星线焦距来源控制**：星轨与延时星轨工作流可选择读取 EXIF 或手动指定焦距和裁切系数；手动模式会明确覆盖 EXIF。
- **CLI 路由参数短名称**：命令行 `--config` 支持使用唯一的 route config 叶节点名称；名称未知或存在歧义时会直接报告可用项，不再静默使用默认值。

### ✅ Bug Fixed

- 修复无法读取 `FocalLengthIn35mmFilm` （35mm 等效焦距）的问题，改善EXIF利用能力；GUI 数值输入支持空值。
- 修复 `uint32` TIFF 通过 OpenCV 保存失败的问题，并补充 `float32` TIFF 保存、RGB/BGR 通道转换及 EXIF 写入路径。
- 修复 `ImageSaveOp` 保存失败后仍广播返回码并使计算图被视为正常完成的问题。
- 修复手动焦距输入的默认值可能覆盖 EXIF 的问题。

---

## v1.1.0-beta.1 (Sep 6th, 2026)

### ✅ New Features

- **统一对齐叠加工作流**：将原有星点对齐、Norma Bundle 对齐和天地分离叠加收口为 `aligned_stack`，可在同一入口选择直接对齐或联合对齐。
- **延时对齐降噪缩星**：延时降噪工作流新增可选缩星处理。
- **大光心偏移求解选项**：对齐时可显式允许更大的主点偏移，用于经过裁切或镜头光心明显偏移的素材。

### ✅ Improvements

- **长序列 BA 加速**：引入抽样求解共享相机-求解全部帧旋转的二阶段BA求解算法，大幅度降低时间和内存开销。
- **BA 稀疏与分层求解**：引入稀疏 Jacobian 和分层旋转初始化/求解，提升大规模序列的收敛速度与稳定性。
- **去卫星线旋转估计更新**：相邻帧匹配切换到星群匹配与仅旋转优化流程，减少直接匹配结果对滑动窗口对齐的影响。

### ✅ Bug Fixed

- 修复 `ImageAddOp` 对不同 dtype 或 `FloatImage`/数组混合输入直接相加时，结果 dtype 和量程语义不一致的问题。

---

## v1.1.0-beta.0 (Sep 1st, 2026)
...
- 修复 GUI 按进度条创建顺序显示“当前节点”，导致等待中的节点长时间遮蔽实际执行状态的问题。
